...
public class MainClass {
    ...
    public static void main(String[] args) throws BrokenBarrierException, InterruptedException {
        ...
        CountDownLatch countLimited1 = new CountDownLatch(CARS_COUNT);
        CountDownLatch countLimited2 = new CountDownLatch(CARS_COUNT);
        CountDownLatch countLimited3 = new CountDownLatch(CARS_COUNT);

        Race race = new Race(new Road(60,countLimited2), new Tunnel(), new Road(40,countLimited3));
        ...
        for (int i = 0; i < cars.length; i++) {
            final int w = i;
            new Thread(()->cars[w].runCar(cb,countLimited1)).start();
        }
        countLimited1.await();
